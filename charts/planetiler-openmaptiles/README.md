...
postMove:
  ...
  runBin: "planetiler" # binary to launch the build when wrapping
```
Notes:
- When `postMove.enabled=true` and you do not set a custom `command`, the chart wraps execution in `/bin/sh -lc` and runs: `planetiler <args> && mv -f <from> <to>`.
...
- If the image requires a different entrypoint than `planetiler`, set `postMove.runBin` accordingly.
